...
// SortUniq sorts and remove duplicates from src and puts the result in dst
// The returned slice is a subslice of dst
// It's the caller responsability to ensure that len(dst) >= len(src)
func SortUniq(dst []string, src []string) []string {
	copy(dst, src)
	elements := dst[:len(src)]
	return SortUniqInPlace(elements)
}
