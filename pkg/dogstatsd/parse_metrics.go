...
func parseMetricSample(interner *stringInterner, message []byte) (dogstatsdMetricSample, error) {
	...
	for message != nil {
		...
		if bytes.HasPrefix(optionalField, tagsFieldPrefix) {
			tags = parseTags(optionalField[1:], interner)
	...
	return dogstatsdMetricSample{
		name:       interner.LoadOrStore(name),
		value:      value,
