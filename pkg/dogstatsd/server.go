...
func NewServer(metricOut chan<- []*metrics.MetricSample, eventOut chan<- []*metrics.Event, serviceCheckOut chan<- []*metrics.ServiceCheck) (*Server, error) {
	...
	packetChans := make([]chan listeners.Packets, workers)

	for i := 0; i < workers; i++ {
		packetChans[i] = make(chan listeners.Packets, config.Datadog.GetInt("dogstatsd_queue_size"))
	}

	socketPath := config.Datadog.GetString("dogstatsd_socket")
	if len(socketPath) > 0 {
		unixListener, err := listeners.NewUDSListener(packetChans, packetPool)
		if err != nil {
	...
	if config.Datadog.GetInt("dogstatsd_port") > 0 {
		udpListener, err := listeners.NewUDPListener(packetChans, packetPool)
		if err != nil {
			log.Errorf(err.Error())
		} else {
	...
	for i := 0; i < workers; i++ {
		go s.worker(packetChans[i], metricOut, eventOut, serviceCheckOut)
	}
