...
func TestParseGauge(t *testing.T) {
	...
	assert.Equal(t, 0, sample.tags.tagsCount)
...
func TestParseCounterWithTags(t *testing.T) {
	...
	assert.Equal(t, countType, sample.metricType)
	assert.Equal(t, 2, sample.tags.tagsCount)
	assert.Equal(t, []byte("protocol:http"), sample.tags.tags[0])
	assert.Equal(t, []byte("bench"), sample.tags.tags[1])
	assert.InEpsilon(t, 1.0, sample.sampleRate, epsilon)
}
...
func TestParseGaugeWithTags(t *testing.T) {
	...
	assert.InEpsilon(t, 666.0, sample.value, epsilon)
	assert.Equal(t, gaugeType, sample.metricType)
	require.Equal(t, 2, sample.tags.tagsCount)
	assert.Equal(t, []byte("sometag1:somevalue1"), sample.tags.tags[0])
	assert.Equal(t, []byte("sometag2:somevalue2"), sample.tags.tags[1])
	assert.InEpsilon(t, 1.0, sample.sampleRate, epsilon)
}
...
func TestParseGaugeWithUnicode(t *testing.T) {
	...
	assert.InEpsilon(t, 666.0, sample.value, epsilon)
	assert.Equal(t, gaugeType, sample.metricType)
	require.Equal(t, 1, sample.tags.tagsCount)
	assert.Equal(t, []byte("intitulé:T0µ"), sample.tags.tags[0])
	assert.InEpsilon(t, 1.0, sample.sampleRate, epsilon)
}
