...
func applyEventOptionalField(interner *stringInterner, event dogstatsdEvent, optionalField []byte) (dogstatsdEvent, error) {
	newEvent := event
	var err error
	switch {
	...
	case bytes.HasPrefix(optionalField, eventTagsPrefix):
		newEvent.tags = parseTags(optionalField[len(eventTagsPrefix):], interner)
	}
	if err != nil {
		return event, err
	}
...
func parseEvent(interner *stringInterner, message []byte) (dogstatsdEvent, error) {
	...
	for optionalFields != nil {
		...
		event, err = applyEventOptionalField(interner, event, optionalField)
		if err != nil {
