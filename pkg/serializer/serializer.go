...
func (s Serializer) serializeStreamablePayload(
	payload marshaler.StreamJSONMarshaler,
	payloadBuilderPolicy jsonstream.PayloadBuilderPolicy) (forwarder.Payloads, http.Header, error) {

	payloads, err := s.seriesPayloadBuilder.BuildWithPolicy(payload, payloadBuilderPolicy)
...
func (s Serializer) serializeStreamJSONMarshalerFactoryPayload(
	...
	eventPayloads, extraHeaders, err := s.serializeStreamablePayload(streamJSONMarshalerFactory.CreateSingleMarshaler(), jsonstream.FailedErrTooBig)
