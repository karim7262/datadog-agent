...
func CollectSystemInfo(cfg *config.AgentConfig) (*model.SystemInfo, error) {
	...
	log.Debugf("CollectSystemInfo - CPU count: %g", len(cpuInfo))
	for _, c := range cpuInfo {
		...
		log.Debugf("CollectSystemInfo - CPU cores: %g", c.Cores)
	}

	log.Debugf("CollectSystemInfo - cpus count: %g", len(cpus))
