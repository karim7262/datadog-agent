...
build do
  ...
  block do
    ...
    if windows?
      ...
      command "#{python} -m piptools compile --verbose --generate-hashes --output-file #{windows_safe_path(install_dir)}\\#{agent_requirements_file} #{static_reqs_out_file}"
    else
      ...
      command "#{python} -m piptools compile --verbose --generate-hashes --output-file #{install_dir}/#{agent_requirements_file} #{static_reqs_out_file}", :env => nix_build_env
    end
